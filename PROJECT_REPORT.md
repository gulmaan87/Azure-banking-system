...
- **Strict Security & Compliance**: Implement a "Deny-by-Default" network posture, zero public internet exposure for compute resources, and enforce organizational compliance through automated Azure Policies.
- **Granular Access Control**: Establish a robust Identity and Access Management (IAM) framework using Microsoft Entra ID (Azure AD) to enforce Role-Based Access Control (RBAC) and the principle of least privilege across different banking departments.
...
- **Micro-segmentation**: Subnets (`/24`) isolate application domains (e.g., accounts, payments, customer, loans, risk, itops).
- **Network Security Groups (NSGs)**: Act as virtual firewalls at the subnet level, strictly dropping all inbound internet traffic. 

### 3.3 Compute Infrastructure
The compute layer consists of customized Windows Server Virtual Machines (6 VMs in total, 3 per region) tailored for specific banking operations.
- **VM Instances**: 
  - East Asia: `accounts-1`, `payments-1`, `customer-1`
  - Southeast Asia: `loans-1`, `risk-1`, `itops-1`
- **Zero Public IPs**: VMs are explicitly barred from having Public IP configurations to prevent external attack vectors.
...
To satisfy stringent financial regulatory requirements, the environment uses Azure Policy to enforce rules and continuously audit resources.
...
The architecture leverages Microsoft Entra ID to implement strict **Least Privilege Access**. The organization is divided into four key departments, with access granted via Group memberships rather than individual assignments:
...
1. **Information Technology (Bank Administrators)**
   ...
   - **Personnel**: Gulmaan (Head of IT), Priya (Cloud Infrastructure Engineer)
...
2. **Risk & Compliance (Security Auditors)**
   ...
   - **Personnel**: Rahul (CISO), Deepa (Compliance Analyst)
...
3. **Application Engineering (Application Developers)**
   ...
   - **Personnel**: Kavya (Senior Software Engineer), Rohan (DevOps Engineer)
...
4. **Data & Analytics (Data Engineers)**
   ...
   - **Personnel**: Ananya (Senior Data Engineer), Vikram (Analytics Engineer)
